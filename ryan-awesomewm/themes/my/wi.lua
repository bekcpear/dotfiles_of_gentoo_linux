...
mywi.sliderbar:connect_signal("widget::redraw_needed", function(w)
  vol_timer:stop()
  muted = ''
  mytl.s_easy_async(string.format(beautiful.dir .. "/volume-control.sh vol %s", w.value))
  ...
  if mywi.sliderbar.handle_color == beautiful.bg_urgent then
    mytl.s_easy_async(beautiful.dir .. "/volume-control.sh mut 1")
    muted = '[M]'
    mywi.volicon:set_image(beautiful.vol_mute_ico)
  else
    mytl.s_easy_async(beautiful.dir .. "/volume-control.sh mut 0")
  end
...
local conn_name       = ''
local getCmd          = "/bin/bash -c 'nmcli c show --active | sed -n 2p'"
local getCmdF         = false
...
temp_timer:connect_signal('timeout', function()
  hwmon_num       = 1
  while temper_c == '' or temper_c == nil do
    temper_c      = mytl.file_read('/sys/bus/pci/drivers/k10temp/0000:00:18.3/hwmon/hwmon' .. hwmon_num .. '/temp1_input', false)
    hwmon_num     = hwmon_num + 1
    if hwmon_num >= 10 then
      naughty.notify({title = "Get CPU temperature 1 error", text = 'Too many tries.', timeout = 0, fg = beautiful.taglist_fg_focus, bg = beautiful.bg_urgent, border_color = beautiful.bg_urgent})
    end
  end
  hwmon_num       = 1
  while temper1_c == '' or temper1_c == nil do
    temper1_c     = mytl.file_read('/sys/bus/pci/drivers/k10temp/0000:00:19.3/hwmon/hwmon' .. hwmon_num .. '/temp1_input', false)
    hwmon_num     = hwmon_num + 1
    if hwmon_num >= 10 then
      naughty.notify({title = "Get CPU temperature 2 error", text = 'Too many tries.', timeout = 0, fg = beautiful.taglist_fg_focus, bg = beautiful.bg_urgent, border_color = beautiful.bg_urgent})
    end
  end
--  fans_c          = ''
--  hwmon_num       = 1
...
--    end
--  end
  temper          = tonumber(temper_c)
  temper1         = tonumber(temper1_c)
  fans            = tonumber(fans_c)
  tempgraph_t:set_text(string.format("%.1f °C, %.1f °C (%d rpm)", temper / 1000, temper1 / 1000, fans))
  temp_graph_prog_c.value = temper / 1000
  if temper >= 70000 and temp_flag ~= 1 then
    temp_flag = 1
  ...
  elseif temper > 60000 and temper < 70000 and temp_flag ~= 2 then
    temp_flag = 2
  ...
  elseif temper <= 60000 and temp_flag ~= 3 then
    temp_flag = 3
...
if beautiful.wallpaper_dir ~= nil or (beautiful.wallpaper_dir_day ~= nil and beautiful.wallpaper_dir_night ~= nil) then
  ...
  beautiful.wallpaper = function(s, noFade)
    ...
    awful.spawn.with_line_callback(string.format("/bin/bash -c 'ls -1 %s | egrep \"\\.(png|jpg|jpeg)$\"'", wall_dir), {
